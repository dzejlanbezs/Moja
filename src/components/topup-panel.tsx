...
export function TopupPanel({
  ...
  const feeCents = validAmount ? cryptoFeeCents(amountCents) : 0;
  const creditCents = validAmount ? amountCents - (method === "crypto" ? feeCents : 0) : 0;
  ...
  async function pay(event: React.FormEvent) {
    ...
    try {
      ...
      const data = (await response.json()) as { error?: string; code?: string; creditCents?: number };
      if (!response.ok) {
      ...
      setDone({
        code: data.code!,
        amountCents,
        creditCents: data.creditCents ?? amountCents,
        method,
      });
    } catch {
    ...
    return (
      <div className="glass-strong mx-auto max-w-xl animate-pop rounded-[30px] p-8 text-center sm:p-12">
        ...
        <h2 className="mt-6 font-display text-4xl">
          {done.method === "crypto" ? "Transfer registered" : "Top-up received"}
        </h2>
        <p className="mx-auto mt-3 max-w-md text-[15px] leading-relaxed text-mist-300">
          {done.method === "crypto" ? (
            ...
            <>
              ...
              <span className="font-medium text-white">{formatPrice(done.amountCents)}</span> is with our team. As
              soon as an admin approves it, the money lands in your balance.
            </>
          )}
        </p>
    ...
    <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr]">
        ...
        <form onSubmit={pay} className="glass-strong rounded-[30px] p-6 sm:p-8">
          ...
          <p className="mt-2 mb-6 text-sm text-mist-500">
            {method === "card"
              ...
              : "Send the transfer from your own wallet — no card needed."}
          </p>
          ...
          <div className="mb-7 flex gap-2 rounded-2xl bg-white/5 p-1.5">
            {(
              [
                ...
                { key: "crypto", label: "Pay with crypto", icon: Bitcoin },
              ] as const
            ).map((tab) => (
              <button
                key={tab.key}
      ...
      <aside className="space-y-5">
        <div className="card p-6">
          ...
          {validAmount && (
            <p className="mt-3 text-sm text-mist-500">
              ...
              {method === "crypto" && step === "card" && " (after the 0.5% crypto fee)"}
            </p>
          )}
        </div>
        ...
        <div className="card space-y-4 p-6 text-sm text-mist-500">
          ...
          <p>Card or crypto: ETH, USDC, USDT, BTC and SOL are all accepted.</p>
